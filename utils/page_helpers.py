# util/page_helpers.py

from playwright.async_api import Page


async def close_popup_if_exists(page: Page,logger):
    """
    关闭典雷达页面可能出现的新手引导 / 弹窗
    不存在则直接跳过
    """
    try:
        close_btn = page.locator(".el-dialog__body .close-btn")
        if await close_btn.count() > 0:
            await close_btn.first.wait_for(state="visible", timeout=3000)
            await close_btn.first.click()
            logger.info("❎ 已关闭引导弹窗2")
            await page.wait_for_timeout(500)
    except Exception:
        pass

async def close_popup_if_exists3(page: Page,logger):
    """
    关闭典雷达页面可能出现的新手引导 / 弹窗
    不存在则直接跳过
    """
    try:
        close_btn = page.locator(".free-dialog .close-icon")
        if await close_btn.count() > 0:
            await close_btn.first.wait_for(state="visible", timeout=3000)
            await close_btn.first.click()
            logger.info("❎ 已关闭引导弹窗3")
            await page.wait_for_timeout(500)
    except Exception:
        pass

async def close_btn_if_exists(page:Page,logger):
    try:
        close_btn = page.locator("#__layout > div > div:nth-child(19) > div > div.el-dialog__body > div > div.close")
        if await close_btn.count() > 0:
            await close_btn.first.wait_for(state="visible", timeout=3000)
            await close_btn.first.click()
            logger.info("❎ 已关闭引导弹窗1")
            await page.wait_for_timeout(500)
    except Exception:
        pass

async def temu_close_popup_if_exists(page: Page):
    try:
        close_btn = page.locator(".arco-icon.arco-icon-close.close-icon")
        # print('检查到的弹窗个数:', close_btn)
        if await close_btn.count() > 0:
            await close_btn.first.wait_for(state="visible", timeout=3000)
            await close_btn.first.click()
            print("❎ 已关闭引导弹窗")
            await page.wait_for_timeout(500)
    except Exception:
        pass