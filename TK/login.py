...
class TikTokLogin:
    ...
    async def login(self):
        ...
        try:
            ...
            # 输入用户名 - 增加多种选择器
            selectors = [
                "#TikTok_Ads_SSO_Login_Mobile_Input",
                "input[name='username']",
                "input[type='text']",
...
async def main():
    with open("./data/tk_accounts.json", "r", encoding="utf-8") as f:
    ...
    for name, data in accounts.items():
        print(f"\n==== {name} ====")
        ok = await TikTokLogin(name, data).run()
        success += ok
        if not ok:
            failed.append(name)
        await asyncio.sleep(3)

    print(f"\n成功 {success}/{len(accounts)}")
    if failed:
        print("失败账户:", failed)
