...
def test_query_records():
    ...
    # 将结果保存到CSV文件
    save_to_csv(all_records, "dingtalk_records.csv")

    # 如果有范围查询的结果，也单独保存
    if all_results:
        save_to_csv(all_results, "filtered_records.csv")

    return all_records


def save_to_csv(records, filename="dingtalk_data.csv"):
    """将记录保存为CSV文件"""
    if not records:
        print(f"没有记录可以保存到 {filename}")
        return
...
def save_to_csv_pandas(records, filename="dingtalk_data_pandas.csv"):
    """使用pandas将记录保存为CSV文件（备选方案）"""
    if not records:
        print(f"没有记录可以保存到 {filename}")
        return
