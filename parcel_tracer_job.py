...
db = pymysql.connect(
    host='rm-bp186omby3lautfn0no.mysql.rds.aliyuncs.com',  # 改为RDS地址
    port=3306,
    user='root_lxz',  # 改为RDS用户名
    password='Lxz123456',  # 改为RDS密码
    database='py_spider',
    charset='utf8mb4',
...
async def fetch_and_upload_delivery_data(shop_name):
    ...
    s_delivery = PickupTrace(shop_name, 'shein_parcel_tracer')
    delivery_items = await s_delivery.fetch_all_pages()

    # 添加检查
    if delivery_items is None:
        logger.warning(f'店铺{shop_name}发货数据返回None，将使用空列表')
        delivery_items = []
...
async def fetch_and_upload_stockin_data(shop_name):
    ...
    s_stockin = StockinManager(shop_name, 'shein_parcel_tracer')
...
async def check_and_send_wechat_for_stockin_differences(shop_name, stockin_items):
    ...
    for item in stockin_items:
        deliver_qty = int(item.get("送货数量", 0))
        receive_qty = int(item.get("上架数量", 0))
        if deliver_qty != receive_qty:
            diff = deliver_qty - receive_qty
            differences.append({
                'purchase_order_sn': item.get("订单号", ""),
                'deliver_qty': deliver_qty,
                'receive_qty': receive_qty,
                'diff': diff,
                'receive_time': item.get("上架时间", ""),
...
def build_wechat_stockin_message(shop_name, differences):
    ...
    for i, diff in enumerate(differences[:], 1):
        ...
        message += f"   送货数量：{diff['deliver_qty']}  上架数量：{diff['receive_qty']}  差异：{diff['diff']}\n"
        if diff['receive_time']:
            message += f"   上架时间：{diff['receive_time']}\n"
...
def prepare_stockin_table_data(stockin_items):
    ...
    records = []
    for item in stockin_items:
        if int(item.get("送货数量", 0)) == int(item.get("上架数量", 0)):
            continue
        ...
        record = {
            ...
            "店铺": item.get("店铺", ""),
            "备货单号": item.get("订单号", ""),
            "送货数量": item.get("送货数量", 0),
            "入库数量": item.get("上架数量", 0),
            "收货时间": item.get("上架时间", "")
        }
        records.append(record)
    return records
...
def query_shop_abnormal_data_from_db(shop_name):
    ...
    try:
        ...
        # 查询该门店的发货异常数据
        cursor.execute("""
            SELECT * FROM `shein_delivery_note_record` 
        ...
        # 查询该门店的入库异常数据
        cursor.execute("""
            SELECT * FROM `shein_purchase_stock_record` 
    ...
    if delivery_abnormals:
        ...
        if delivery_abnormals:
            ...
            for i, row in enumerate(delivery_abnormals[:]):
                ...
                delivery_summary += f"       状态: {row['order_status']} -> {row['mark_status']}\n"
                if row['mark_reason']:
...
def send_shop_messages(shop_data):
    ...
    for shop_name, delivery_count, stockin_count, message in shop_data:
        if "未发现异常数据" not in message:
            # 发送到钉钉群
            ding_bot_send('Shein_Logistics_Exception_Monitor', message)
...
async def main():
    ...
    ding_bot_send('me', f'Shein揽收丢件任务结束,总耗时：{format_seconds(total_cost)}')
