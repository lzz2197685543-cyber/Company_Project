...
class AutoListing:
    ...
    def get_detial_id(self):
        ...
        for i in data['detailList']:
            ...
            items.append(detail_id)

        return items

    def selectShop(self, detailid):
        self.logger.info(f"detailid是: {detailid}")
        """选择店铺"""
        data = {
        ...
        self.logger.info(f"选择店铺响应：{response.json()}")
        if response.json()['result'] == 'success':
            self.logger.info('选择店铺成功')
        else:
            self.logger.info('选择店铺失败')
    ...
    def publish(self, detailid):
        ...
        self.logger.info("发布响应",response.json())
        if response.json()['result'] == 'success':
            self.logger.info('发布成功')
        else:
            self.logger.info("发布失败")

    async def run(self):
        await self.get_cookie()
        # 1.获取采集箱id
        detailid_list = self.get_detial_id()
        self.logger.info(detailid_list)
        self.logger.info(len(detailid_list))


        for detailid in detailid_list:
            # 3.选择店铺
            self.selectShop(detailid)
            # 4.获取商品信息
            goods_info = self.get_goods_info(detailid)
            # 5.修改并保存商品信息（会自动判断是否需要添加说明书）
            self.save(goods_info)
            # 6.点击发布
            self.publish(detailid)
