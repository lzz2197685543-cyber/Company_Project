...
class Temu_skc_sku:
    ...
    'seller_temp': 'N_eyJ0IjoieUI2dkFkcENXWERBdFJ3VlIyMXRkamxobmM3TVUvQk5Yd2pJOFZsMEt0d09NSFNyRUlVV0tZaDN2QVZ1a0xDMCIsInYiOjEsInMiOjEwMDAxLCJ1Ijo5MzgyMjA3MjU3NzkyfQ==',
        ...
        self.url = 'https://agentseller.temu.com/api/kiana/mms/robin/queryPunishInfo'

    def get_info(self):
        json_data = {
            'productId': 1024882288,
        }
    ...
    def parse_data(self, json_data):
        ...
        for i in json_data['result']['punishInfoList']:
            skc = i.get('typeId')
            site=i.get('site')
            reason=i.get('reason')

            site_violations['SKC'].append(skc)
            site_violations['站点'].append(site)
            site_violations['违规原因'].append(reason)
    ...
    def save_items(self, data):
        ...
        fname = out_dir / f"{self.shop_name}_skc_site_violate_{datetime.now():%Y%m%d}.csv"
        exists = fname.exists()

        with open(fname, "a", newline="", encoding="utf-8-sig") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["SKC", "站点", "违规原因"]
            )
